...
`;
  ...
  function fetchImage(src, { timeout = 30e3, useProxy = "" } = {}) {
  ...
  function deepClone(node, styleMap, styleCache, nodeMap, compress, options = {}, originalRoot) {
    ...
    if (node.shadowRoot) {
      const shadowContent = Array.from(node.shadowRoot.children).filter((el) => el.tagName !== "STYLE").map((el) => deepClone(el, styleMap, styleCache, nodeMap, compress, options, rootElement)).filter(Boolean);
